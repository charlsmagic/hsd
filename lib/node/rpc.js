...
class RPC extends RPCBase {
  ...
  constructor(node) {
    ...
    this.merkTreeMap = new BufferMap();
    this.pollers = [];
  ...
  async handleWork(data) {
    ...
    try {
      ...
      const workData = Buffer.from(data, 'hex');

      return await this._handleWork(workData);
    } finally {
      unlock();
    }
  }
  ...
  async _handleWork(data) {
    ...
    const block = Block.fromMiner(data)
    block.txs.push(attempt.coinbase);
    ...
    try {
      ...
      this.merkTreeMap.clear();
    } catch (err) {
      if (err.type === 'VerifyError') {
        this.logger.warning('RPC block rejected: %x (%s).',
          block.hash(), err.reason);

        if (this.merkTreeMap.has(block.merkleRoot)){
          const block1 = Block.fromMiner(data)
          block1.txs.push(attempt.coinbase);

          const itemTxs = this.merkTreeMap.get(block.merkleRoot)

          for (const item of itemTxs)
            block1.txs.push(item.tx);

          let entry1;
          try {
            entry1 = await this.chain.add(block1);
          } catch (err) {
            this.logger.warning('RPC block rejected: %x (%s).',
              block1.hash(), err.reason);
          }
        }
  ...
  async _createWork() {
    ...
    this.merkTreeMap.set(attempt.merkleRoot, attempt.items)
